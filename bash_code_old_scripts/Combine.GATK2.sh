...
java -Xmx2g -jar GenomeAnalysisTK.jar -R NM3352.fasta -T CombineVariants --variant $i.filtered.vcf --variant combined.vcf -o tmp.vcf

mv tmp.vcf combined.vcf;
...
~/vcftools/bin/vcftools --vcf output.vcf --remove-indels --remove-filtered-all --recode --out combined

vcf-to-tab <combined.recode.vcf >tmp2.tab

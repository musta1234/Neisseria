...
filters="+/Q=30/c=15,1000"

# c=5,1000 filters any SNPs with density > 5 SNPS/kb

 

for i in  $(ls *.raw.vcf| sed 's_\.raw\.vcf__g' ) ; 

	do

 	vcf-annotate -f "$filters" < ./"$i".raw.vcf  > ./"$i".nohgt.vcf;
	
	~/vcftools/bin/vcftools --vcf "$i".nohgt.vcf --remove-indels --remove-filtered-all --recode --out $i; 
...
rm -f *.tab *.recode.vcf *.log *.vcfidx *.nohgt.vcf 
